  ...
  return rewards ? (
    <div className={classes.root}>
      <div style={{ width: '100%' }}>
        ...
        {rewards.map((r: RewardsReportWithoutDelegations, index: number) => {
          ...
          return (
            ...
            >
              <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                ...
                <Typography className={classes.heading}>
                  {utils.parseTEZWithSymbol(Number(r.staking_balance))}
                </Typography>
                <Typography className={classes.heading}>
                  {utils.parseTEZWithSymbol(Number(r.rewards))}
                </Typography>
                <Typography className={classes.heading}>
                  {utils.parseTEZWithSymbol(Number(r.fees))}
                </Typography>
              ...
              <ExpansionPanelDetails>
                {expanded === index ? (
                  <RewardsPerDelegator
                    ...
                    paymentsAllowed={index > 4 && !rewarder.active}
                    pkh={pkh}
                    cycle={r.cycle}
                  />
                ) : (
