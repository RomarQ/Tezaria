...
export default ({ userData }: Props) => {
  ...
  React.useEffect(() => {
    ...
    const delay =
      rpc.networkConstants.time_between_blocks && !bakerInfo.waitingForRights
  ...
  const updateInfo = () => {
    utils
      .verifyNodeCommits()
      .then(commitState => {
        if (isMounted.current && commitState) setTezosCommits(commitState)
      })
    ...
    bakingController
      .load(userData.keys)
      .then(delegate => {
        if (isMounted.current && delegate) { setBakerInfo({ ...userData, ...delegate }) }
      })
      .catch(e => console.error(e))
  }
